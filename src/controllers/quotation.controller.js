...
exports.createQuotation = async (req, res) => {
  try {
    ...
    if (orderId) {
       ...
       const { calculatePricing } = require('../utils/pricing.util');
       ...
       if (cart) {
         cart.quotationId = savedQuotation._id;
         cart.quotationCost = savedQuotation.totalAmount;

         cart.tracking.push({
           message: 'Quotation generated by partner. Pending customer approval.',
           status: cart.status,
           date: new Date()
         });
         await cart.save();
    ...
    ably.channels.get(`user-${userId}`).publish("quotation_generated", {
    ...
    // Notify Admin
    ably.channels.get("admin-channel").publish("quotation_updated", {
      message: "New quotation generated by partner",
      requestId: orderId
    });
...
exports.updateQuotation = async (req, res) => {
  try {
    ...
    if (quotation.orderId) {
       ...
       if (cart) {
         cart.quotationId = updatedQuotation._id;
         cart.quotationCost = updatedQuotation.totalAmount;

         cart.tracking.push({
           message: 'Quotation updated by partner. Pending customer approval.',
           status: cart.status,
           date: new Date()
         });
         await cart.save();
...
exports.acceptQuotation = async (req, res) => {
  try {
    ...
    if (quotation.orderId) {
       ...
       const Cart = require('../models/cart.model');
       // We don't use calculatePricing here because it overwrites checkout-distributed discounts.
       // The coupon/service discount is already proportionally distributed and locked in.
       // We just add the quotation items cost securely.
       ...
       if (cart) {
         // Convert quotation total to additionalItemsCost
         cart.additionalItemsCost = quotation.totalAmount;
         cart.quotationCost = quotation.totalAmount; 
         
         // Securely tally all the costs without losing the discount amount
         cart.calculateTotals();

         cart.tracking.push({
           message: 'Quotation accepted by customer. Order total updated.',
    ...
    ably.channels.get("admin-channel").publish("quotation_updated", {
      ...
      requestId: quotation.orderId,
      status: 'Accepted'
    });

    ably.channels.get(`partner-${quotation.partnerId}`).publish("quotation_updated", {
